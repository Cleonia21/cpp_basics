...
void Form::chekToExecute(Bureaucrat const &bureaucrat) const
{
	if (!this->_flag)
		throw std::runtime_error("Form::ExceptionFormNotSigned");
	if (bureaucrat.getGrade() > _requiredAchieve)
		throw std::runtime_error("Form::GradeTooLowException");
}
